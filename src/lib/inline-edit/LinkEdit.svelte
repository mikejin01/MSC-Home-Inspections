<script>
	...
	let {
		...
		defaultHref = '#',
		/**
		 * Prefix applied to hrefs that are bare `#anchor` fragments. The home
		 * page leaves this empty; every other route passes the home URL, so a
		 * nav or footer link to `#services` still lands on the section instead
		 * of dead-ending on the current page.
		 */
		anchorBase = '',
	...
	const raw = $derived(wpEdit.text(urlKey, defaultHref));
	const href = $derived(anchorBase && raw.startsWith('#') ? `${anchorBase}${raw}` : raw);
